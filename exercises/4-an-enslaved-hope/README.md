...
2. Fill in the Git settings with your `todolist-api` GitLab url and set the credentials as you've done before. `https://gitlab.apps.<OpenShift cluster url><YOUR_NAME>/todolist-api.git`
...
3. Head to <OpenShift cluster url> on OpenShift and move to your ci-cd project > builds. You should see `jenkins-slave-zap` has been built.
...
```
SOURCE_REPOSITORY_URL=https://gitlab.apps.<OpenShift cluster url><GIT_USERNAME>/enablement-ci-cd.git
...
3. Head to <OpenShift cluster url> on OpenShift and move to your ci-cd project > builds. You should see  `jenkins-slave-arachni`.
